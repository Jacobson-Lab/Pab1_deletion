...
# Plot RNA vs protein levels for decay factors --> code in Plot_Figure2_S2_RNAseq_MassSpec.R

# Combine plots
library(patchwork)
pA <- ps + labs(tag = "A") + psr + 
  plot_layout(nrow = 2, heights = c(0.2, 0.8)) & 
  theme(plot.margin = unit(c(0, 0, 0, 0), "cm"), legend.position = "none")
pB <- (plot_prot_group_decay + labs(tag = "B") + theme(legend.position = "bottom") + 
         guides(color = guide_legend(title = "mRNA vs. protein changes:", override.aes = list(alpha = 1))))
p_fig6 <- pA / pte / pB +
...
cairo_pdf(filename = "Fig6_decay_substrates.pdf", family = "Arial", width = 7.5, height = 9) 
p_fig6
dev.off()
