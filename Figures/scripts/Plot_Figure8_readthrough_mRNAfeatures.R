...
stat.test <- bp_filter %>%
  ...
  add_significance()
stat.test <- stat.test %>% add_xy_position(x = "strain", step.increase = 0.2)

bp_filter$dummy_facet <- "" # This is to avoid gap between y-axis ticks and plot border from trying to align plots by patchwork
p_rt <- ggplot(bp_filter, aes(x = strain, y = log_rte)) +
  ...
  stat_pvalue_manual(data = stat.test, label = "p.adj.signif", tip.length = 0.01, size = 7/.pt, vjust = 0.7, hide.ns = TRUE) +
  scale_fill_manual(name = "", values = set3[c(4, 7, 5)]) +
  facet_wrap(~dummy_facet, nrow = 1) +
  xlab("") + ylab("Readthrough\nefficiency") +
...
df_reg <- read.table("../data/pab1_rf_reg_5foldCV_5repeats_feature_importance.txt", sep = "\t", header = TRUE)
...
df_plot <- read.table("../data/pab1_re_reg_l_utr3_spearman.txt", header = TRUE, sep = "\t")
...
# Combine plots
library(patchwork)
p <- (p_rt | plot_spacer()) / p_reg / (p_lutr3 + theme(axis.title.y = element_text(margin = margin(r = -30, unit = "pt")))) + 
  plot_layout(heights = c(3.5, 1, 4), guides = 'keep') + 
...
cairo_pdf(filename = "Fig8_readthrough_mRNA_features.pdf", family = "Arial", width = 7.5, height = 7.5) 
p
dev.off()
