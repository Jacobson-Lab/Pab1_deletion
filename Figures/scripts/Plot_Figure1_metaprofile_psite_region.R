...
# A: Metaprofile
df <- read.table("../data/Fig1A_metaprofile.txt", header = TRUE, sep = "\t")
...
p_inset <- ggplot() +
  ...
  theme_bw(base_size = 10) + 
  theme(panel.grid = element_blank(), axis.text = element_blank(), axis.ticks = element_blank(), axis.title = element_blank(), axis.ticks.length = unit(0, "pt"),
        legend.position = "none", strip.text = element_blank(), panel.spacing.y = unit(0, "cm"), plot.margin = grid::unit(c(0, 0, 0, 0), "mm"))
...
fr <- data.table(read.csv("../data/Fig1BC_psite_region_frame.csv"))
...
# D: Readthrough efficiency
bp <- read.csv("../../Analysis scripts/Readthrough/pab1_strains_readthrough_efficiency.csv")
bp$strain <- recode_factor(bp$strain, WT = "WT", pbp1d = "pbp1\u0394", pab1d = "pab1\u0394pbp1\u0394")
bp_filter <- bp[which(bp$rpkm_cds > 2 & bp$rpkm_ext > 0.1), ]
bp_filter$log_rte <- log2(bp_filter$rte_ext)
nn <- data.frame(table(strain = bp_filter$strain))
stat.test3 <- bp_filter %>%
  wilcox_test(log_rte~strain, p.adjust.method = "BH", paired = FALSE,
              comparisons = list(c("WT", "pbp1\u0394"), c("pbp1\u0394", "pab1\u0394pbp1\u0394"), c("WT", "pab1\u0394pbp1\u0394"))) %>%
  add_significance()
stat.test3 <- stat.test3 %>% add_xy_position(x = "strain", step.increase = 0.2)

bp_filter$dummy_facet <- "" # This is to avoid gap between y-axis and plot border from trying to align plots by patchwork
pD <- ggplot(bp_filter, aes(x = strain, y = log_rte)) +
  geom_boxplot(aes(fill = strain), outlier.alpha = 0.5, outlier.size = 0.5, size = 0.5) +
  geom_text(data = nn, aes(x = strain, y = floor(min(bp_filter$log_rte))-1, label = paste0("n = ", Freq)), size = 7/.pt, color = "black") +
  stat_pvalue_manual(data = stat.test3, label = "p.adj.signif", tip.length = 0.01, size = 7/.pt, vjust = 0.7, hide.ns = TRUE) +
  scale_fill_manual(name = "", values = set3[c(4, 7, 5)]) +
  facet_wrap(~dummy_facet, nrow = 1) +
  xlab("") + ylab("Readthrough efficiency") +
  theme_bw(base_size = 10) + 
  theme(strip.background = element_blank(), strip.text = element_blank(), strip.placement = "outside", panel.grid = element_blank(),
        legend.position = "right", legend.text = element_text(face = "italic"),
        axis.text.x = element_text(face = "italic"), axis.title.x = element_blank())
...
D#
"
p <- (pA + pB + pC + pD) + plot_layout(guides = 'keep', design = design) +
...
cairo_pdf(filename = "Fig1_metaprofile.pdf", family = "Arial", width = 7.5, height = 10) 
p
dev.off()
