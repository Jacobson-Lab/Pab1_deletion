...
analyze_costello <- function(ud, IP_data = "eIF4G1") {
  cols = c("transcript", "l_tr", "l_cds", "l_utr5", "l_utr3", "rpkm_cds", "rpkm_ext", "rte_ext", "TE_change_PPvsP", "baseMean", "log2FoldChange")
...
```{r}
# Plot logFC
bb <- bind_rows(lapply(res_ripseq, function(x) x$df), .id = "IP")
p_ripseq_scatter <- ggplot(bb, aes(x = log2FoldChange, y = logFC)) + 
  geom_point(alpha = 0.3, color = "grey50") + facet_grid(.~IP) + 
  stat_cor(method = "spearman", cor.coef.name = "rho", size = 7/.pt) + geom_smooth(method = "lm") +
...
```
```{r}
comp <- get_comparisons(bbf, variable = "TE_change_PPvsP")
stat.test7 <- bbf %>% group_by(IP, dir) %>% 
  wilcox_test(l_tr ~ TE_change_PPvsP, comparisons = comp, p.adjust.method = "BH")
stat.test7 <- stat.test7 %>% add_significance() %>% add_xy_position()
stat.test7$y.position <- rep(c(log10(6000), log10(7000), log10(8000)), times = 9)

nn7 <- data.frame(table(TE_change_PPvsP = bbf$TE_change_PPvsP, IP = bbf$IP, dir = bbf$dir))

p_ripseq4 <- ggplot(bbf, aes(x = TE_change_PPvsP, y = l_tr)) +
  geom_boxplot(outlier.alpha = 0, aes(fill = TE_change_PPvsP)) +
  geom_text(data = nn7, aes(x = TE_change_PPvsP, y = 200, label = Freq), size = 7/.pt, position = position_dodge(width = 0.2)) +
  stat_pvalue_manual(data = stat.test7, label = "p.adj.signif", tip.length = 0.002, size = 7/.pt, vjust = 0.5, hide.ns = TRUE) +
  facet_nested(.~IP+dir) +
  scale_fill_manual(name = "TE changes: ", values = c(Up = "orange", Down = "purple", Ref. = "white")) +
  scale_y_log10() +
  xlab("") + ylab("mRNA length (nt)") +
  theme_bw(base_size = 10) + 
  theme(strip.background = element_rect(fill = "white"), strip.placement = "outside",
...
cairo_pdf(filename = "FigS7.pdf", family = "Arial", width = 7.5, height = 5) 
p_nt_all
dev.off()
```
...
```{r}
p_rip_trl <- (p_ripseq3 / p_ripseq4) +
  plot_layout(guides = "keep") +
  plot_annotation(tag_levels = "A") & theme(plot.tag = element_text(size = 12, face = "bold"), plot.tag.position = "topleft", legend.position = "top")

cairo_pdf(filename = "FigS8.pdf", family = "Arial", width = 7.5, height = 7.5) 
p_rip_trl
dev.off()
```
```{r}
